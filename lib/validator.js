...
const isDib = (buffer) => {
  ...
  if (Buffer.compare(s.DIB_0, buffer.subarray(0, s.DIB_0.length)) != 0
    && Buffer.compare(s.DIB_1, buffer.subarray(0, s.DIB_1.length)) != 0) {
    return false;
  }
...
const isSwf = (buffer) => {
  ...
  if (Buffer.compare(s.SWF_0, buffer.subarray(0, s.SWF_0.length)) != 0
    && Buffer.compare(s.SWF_1, buffer.subarray(0, s.SWF_1.length)) != 0) {
    return false;
  }
